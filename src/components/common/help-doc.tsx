...
const docsMap: { [key: string]: Promise<typeof import('*.md')> } = {
  // common
...
export async function getDoc(key: string) {
  // check key exists
  // get module from docsMap ignore case
  let module = docsMap[key.toLowerCase()]
  if (!module) module = docsMap['known-address']
  // load module
