import { TaskStatus, type TaskReward, type Task_New } from '@/apis/task.api'
import ImageLocked from '@/assets/task/locked.png'
import { useDuration } from '@/components/common/countdown'
...
const TaskCard = ({
  ...
  const checkDisplayRightBg = (task) => {
  ...
  return (
    ...
    >
      <div
        className={cn(
          'w-full rounded-3xl bg-gray-100 p-6',
          task.locked && 'bg-gray-200'
        )}
      >
        <div className="flex w-full items-end gap-6">
          <div className="flex max-w-[65%] flex-col gap-2">
            ...
            <p className="mb-3 line-clamp-2 leading-normal text-gray-500">
              {task.description}
            </p>
          </div>
            ...
            <Flex gap={8}>
              ...
              {!task.refresh_time && task.start_time && task.expire_time && (
                <CountDown
                  ...
                  endTime={task.expire_time}
                />
              )}
              {task.max_count && (
                <CompleteProgress
                  current={
                    [TaskStatus.Finished, TaskStatus.Rewarded].includes(
                      task.status
                    )
                      ? task.max_count
                      : task.current_count
...
function CountDown(props: {
  ...
  const totalDuration = useMemo(
    () =>
      dayjs
        .duration(dayjs.utc(props.endTime).diff(dayjs.utc(props.startTime)))
        .asSeconds(),
    [props.startTime, props.endTime]
  )
  const percent =
    100 - Math.round(((totalDuration - dur.asSeconds()) / totalDuration) * 100)
  return (
    <div className="flex items-center">
      <img
        className="relative z-[1] mr-[-9px]"
        src={AlarmClock}
        width={32}
        height={32}
        alt=""
...
function CountDownPie(props: {
  refresh_time: number
  onTimeout: () => void
  duration: number
}) {
  const { refresh_time, onTimeout, duration } = props
  const [refershTime] = useState(refresh_time)

  const [len, setLen] = useState(calcLenth())
  const dur = useDuration((refresh_time + 2) * 1000)
  const durFormat = dur.days() >= 1 ? 'D[d] H[h] m[m]' : 'H[h] m[m] s[s]'

  function calcLenth() {
    const offsetSecond = refershTime - Math.round(Date.now() / 1000)
    const percent = offsetSecond / duration
    return 240 * (1 - percent)
  }
...
const TaskProgress = (props: {
  ...
  function renderRateItems(count) {
