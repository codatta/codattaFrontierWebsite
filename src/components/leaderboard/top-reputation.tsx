...
import useTRankStore from '@/stores/rank.store'
...
function UserRank(props: { rank: number; className: string }) {
  ...
  return (
    <div className={`relative w-[112px] shrink-0`}>
      ...
      {rank > 3 && (
        <div className="ml-6 w-9 text-center">{rank > 99 ? '99+' : rank}</div>
      )}
    </div>
  )
}
...
export default function Component() {
  const { reputationRank } = useTRankStore()

  return (
    <div className={`relative w-full rounded-2xl text-white`}>
      <div className="mb-1 mt-6 flex w-full items-center justify-end gap-6 border-b border-b-white/20 py-2 text-sm font-normal text-gray-400">
        <div className="relative w-[100px] shrink-0 text-center">
          <div className="ml-6 w-9 text-center">#</div>
        </div>
        <div className="relative mr-auto h-[22px] w-9 shrink-0 text-sm">
          User
        </div>
        <div className="my-2 flex flex-1 items-center overflow-hidden text-ellipsis text-left"></div>
        <div className="inline-flex w-[108px] gap-0.5">Reputation</div>
      </div>
          ...
          reputationRank.map((user) => (
            <li
              ...
              className={cn(
                ...
                user.rank === 1 &&
                  'bg-gradient-to-r from-[#FCC800]/[0.16] to-40%',
                user.rank === 2 &&
                  'bg-gradient-to-r from-[#CCCCCB]/[0.16] to-40%',
                user.rank === 3 &&
                  'bg-gradient-to-r from-[#FC8800]/[0.16] to-40%'
              )}
            >
              ...
              >
                {user.email ||
                  truncateStr(user.user_id, { len: 10, ellipsis: '***' })}
              </div>
              <ReputationRate
                gap={2}
                size={20}
                color="#FFA800E0"
                rate={user.reputation}
              ></ReputationRate>
            </li>
          ))
        )}
