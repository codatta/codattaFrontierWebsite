...
const Table = () => {
  ...
  return (
    <div className="mt-4 flex min-h-[372px] w-full rounded-3xl bg-white/10 p-6 text-white/90">
      ...
      <div className="flex-1 leading-[40px]">
        ...
        <div>
          <Row
            gutter={16}
            className="mt-[17px] h-10 text-sm font-medium leading-10"
          >
            <Col span={14}>Name</Col>
            <Col
              span={4}
              className="flex w-24 flex-none items-center justify-center text-center"
            >
              Reward
            </Col>
          ...
          <div className="max-h-[200px] overflow-y-auto pb-4">
            {list.length === 0 ? (
              ...
              (loadingMore ? list.slice(0, defaultCount) : list).map(
                (item, index) => (
                  <Row
                    key={item.user_id}
                    gutter={16}
                    className="flex w-full items-center border-b-4 border-solid border-[#300040]/[0.06] text-sm leading-10"
                  >
                    ...
                    <Col className="flex w-24 flex-none items-center justify-center">
                      <div className="h-[22px] rounded-2xl bg-primary/20 px-2 text-xs font-medium leading-[22px] text-primary">
                        {item.reward} points
                      </div>
                    </Col>
                    <Col className="w-[164px] flex-none text-right">
        ...
        <div className="mt-[17px] flex justify-center">
          {loadingMore && (
            <Button onClick={() => setLoadingMore(false)}>Load more</Button>
          )}
        </div>
      </div>
    </div>
