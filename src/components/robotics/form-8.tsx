import { Button, Form, message, Upload, Modal, FormInstance } from 'antd'
...
interface KeyItemInterface {
  key: string
  label: string
  fileListKey: UploadFile[]
  setFileListFn: Dispatch<SetStateAction<UploadFile[]>>
  uploadMsg: string
}
...
const uploadButton = (msg?: string) => (
  <div className="flex size-full cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-[#FFFFFF1F] bg-transparent hover:border-blue-500">
    ...
    <div className="mt-3 flex text-sm text-white">
      {msg || 'Upload an NFT image (only one image allowed). Ensure the image is clear and without watermark.'}
    </div>
  </div>
)
...
function UploadComponent(props: {
  ...
  onPreviewFn: (file: UploadFile) => Promise<void>
}) {
  const { formIns, formKey, fileList, setFileListFn, uploadMsg } = props.info
  const { onPreviewFn } = props
  ...
  const handleChange: UploadProps['onChange'] = ({ fileList: newFileList }) => {
    ...
    formIns.setFieldsValue({ [formKey]: imageUrls })
    setFileListFn(updatedFileList)
  }
  ...
  return (
    ...
    >
      {fileList.length < 1 && uploadButton(uploadMsg)}
    </Upload>
  )
}
...
export default function Component({ onSubmit }: { onSubmit: (data: object) => Promise<unknown> }) {
  ...
  const keyArray: Array<KeyItemInterface> = [
    {
      key: 'top_image',
      label: 'Top Image',
      fileListKey: fileList1,
      setFileListFn: setFileList1,
      uploadMsg: "Upload a standalone front-facing photo of today's top. Ensure good lighting."
    },
    {
      key: 'bottom_image',
      label: 'Bottom Image',
      fileListKey: fileList2,
      setFileListFn: setFileList2,
      uploadMsg: "Upload a standalone front-facing photo of today's bottom. Ensure good lighting."
    },
    {
      key: 'full_outfit_image',
      label: 'Full Outfit Image',
      fileListKey: fileList3,
      setFileListFn: setFileList3,
      uploadMsg:
        "Upload a front-facing full-body photo of today's outfit. Ensure the person is fully visible and well-lit."
    }
  ]
              ...
              <div>
                <UploadComponent
                  info={{
                    formIns: form,
                    formKey: keyItem.key,
                    fileList: keyItem.fileListKey,
                    setFileListFn: keyItem.setFileListFn,
                    uploadMsg: keyItem.uploadMsg
                  }}
