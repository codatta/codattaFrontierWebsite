...
export default function TwitterFollow(props: { templateId: string }) {
  ...
  const linkUrl = useMemo(() => {
    ...
    return locationState?.link || params.get('link') || 'https://twitter.com/intent/follow?screen_name=codatta_io'
    ...
    <div className="min-h-screen bg-[#050011] text-white">
      <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-4 py-8">
        <div className="space-y-3 rounded-3xl border border-white/10 bg-white/5 p-6 text-center">
          <h1 className="text-2xl font-bold">Follow on Twitter</h1>
          <p className="text-sm text-white/70">
            Complete opens the Twitter intent so you can tap Follow. After following, click Verify.
          </p>
          {linkUrl && (
