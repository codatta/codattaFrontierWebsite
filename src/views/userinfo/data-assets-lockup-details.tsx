...
import TokenUnlockModal from '@/components/settings/token-unlock-modal'
...
export default function LockupDetails() {
  ...
  const [tokenIdsToClaim, setTokenIdsToClaim] = useState<string[]>([])
  ...
  const handleClaim = () => {
    if (tokenIdsToClaim.length === 0) {
      message.info('No rewards to claim')
      return
    }
  ...
  const fetchLockups = useCallback(async () => {
    ...
    try {
      ...
      if (Number(count) > 0) {
        ...
        const tokenIds = groupedList
          .filter((group) => group.status === 'To Claim')
          .flatMap((group) => group.items.map((item) => item.uid))

        console.log('tokenIds to claim:', tokenIds, groupedList)
        setTokenIdsToClaim(tokenIds)
  ...
  return (
    <>
      ...
      <TokenUnlockModal
        ...
        tokenIds={tokenIdsToClaim}
        tokens={tokensToClaim as []}
        onClose={() => {
