...
import campaignApi, { CampaignRevealed } from '@/apis/campaign.api'
...
export default function Component() {
  ...
  const [activity, _, loadData] = useRequest(taskApi.getActivity)
  const [sortedTasks, setSortedTasks] = useState<Task_New[]>([])
  const navigate = useNavigate()

  function getActivity(categoryId: string = '') {
    loadData(categoryId)
      .then((response: Activity | null) => {
        if (!response) return
        ReactGA.event('view_quest_list', {
          customParams: JSON.stringify({
            list: response.tasks?.map(
  ...
  useEffect(() => {
    const tasks = sortBy(
      activity?.tasks ?? [],
      (task) => taskStatusSort[task.status]
    )
    setSortedTasks(tasks)
  }, [activity])

  function handleBroacastMessage() {
    ...
    messageApi.success({
      content: (
        <span className="font-medium text-[#020008E0]">
          You have earned{' '}
          {rewards
            .map((reward) => reward.reward_value + ' ' + reward.reward_type)
            .join(', ')}
        </span>
      ),
      icon: (
  ...
  const afterAction = async () => {
    try {
      ...
      await loadConsult(CampaignRevealed.Task, categoryId ?? '')
    } catch (err: unknown) {
  ...
  const [consult, , loadConsult] = useRequest(campaignApi.getConsult)

  useEffect(() => {
    setLoading(true)
    loadConsult(CampaignRevealed.Task, categoryId)
      .catch((err) => {
        message.error(err.message)
      })
      .finally(() => {
...
function Summary(props: {
  activity: Activity | null
  id: Activity['sub_cate_id']
  compact?: boolean
}) {
  const { groups } = useSnapshot(activityStore)
  const { activity } = props

  const activitySummary = groups
    ?.flatMap(({ sub }) => sub)
    .find(({ sub_cate_id }) => sub_cate_id === props.id)
  ...
  if (props.compact)
    return (
      <div className="mx-auto flex flex-col items-center gap-2 text-center">
        <h1 className="font-mona text-[32px] font-bold leading-8">
          {activitySummary.sub_cate_name}
        </h1>
      </div>
    )
  ...
  return (
    <Space direction="vertical" size={24}>
      <h1 className="text-[32px] font-bold leading-8">
        {activitySummary.sub_cate_name}
      </h1>
      ...
      <Space size={8}>
        ...
        <div className="pb-1 text-xs text-gray-700">
          {activitySummary.completed_count} completed
        </div>
      </Space>
